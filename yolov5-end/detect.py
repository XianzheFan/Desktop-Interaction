            ...
            if len(det):
                ...
                if len(INFO) != 0:
                    ...
                    for j in range(len(col_idx)):
                        ...
                        if col_idx[j] >= 0:
                            if int(INFO[ID[j]][4]) == 2 | int(INFO[ID[j]][4]) == 3:
                ...
                # l =len(XYXY)
                # if l == 0: # no device in last frame
                #     continue
                # for *xyxy, conf, cls in reversed(det):
                #     if (cls != 0) & (cls != 1): # only for Pad and Phone
                #         continue
                #     flag = -1
                #     for index, xyxy0 in enumerate(XYXY):
                #         for j in range(4):
                #             if (xyxy0[j] < 0.9*xyxy[0]) | (xyxy0[j] > 1.1*xyxy[0]):
                #                 break
                #             if j == 3:
                #                 flag = index
                #         if flag != -1:
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default='datasets/test/WIN_20220322_16_51_01_Pro.mp4', help='file/dir/URL/glob, 0 for webcam')
    # 'datasets/test/WIN_20220322_16_51_01_Pro.mp4'

...
from track.hungarian import Hungarian, Brightness
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    # camera
    DIM=(1920, 1080)
    K=np.array([[478.1077289560713, 0.0, 984.3163602182244], [0.0, 478.6730943752726, 578.6279375971117], [0.0, 0.0, 1.0]])
    D=np.array([[-0.07708370132419824], [-0.006780842661806889], [0.00027380658705086655], [0.0004692764252071314]])
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            w,h = imMP.shape[1],imMP.shape[0]

            t_undistort = time_sync()
            map1, map2 = cv2.fisheye.initUndistortRectifyMap(K, D, np.eye(3), K, DIM, cv2.CV_16SC2)
            imMP = cv2.remap(imMP, map1, map2, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)
            print("Undistort: %.2fms"%((time_sync()-t_undistort)*1E3))
            ...
            t_MP += (time_sync()-t_mp)
            print("MediaPipe hands & face_mesh: %.4fs" % (time_sync()-t_mp))
            frames += 1
            ...
            if len(det):
                ...
                if len(INFO) != 0:
                    ...
                    # print("LOC: ",LOC)
                    # print("Loc: ",Loc)
                    match = np.zeros(len(Loc)) # Loc to INFO
                    state = np.zeros(len(Loc)) # ON or OFF
                    row_idx, col_idx = Hungarian(LOC,Loc) #Loc to LOC, LOC to Loc
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='runs/train/exp17/weights/best.pt', help='model path(s)')
    parser.add_argument('--source', type=str, default='datasets/test/WIN_20220330_12_33_41_Pro.mp4', help='file/dir/URL/glob, 0 for webcam')
    # 'datasets/test/WIN_20220330_12_33_41_Pro.mp4'

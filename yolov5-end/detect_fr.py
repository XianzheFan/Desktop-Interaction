...
from FaceRecognition.FaceRecog import faceRecognizer
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    situation2 = [-1,0] # user appears in the middle of image. person id, count
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            # face recognition
            if user_id < 0: # user unidentified
                face_locations, face_names = faceRecognizer(imFR)
                for (top, right, bottom, left), name in zip(face_locations, face_names):
                    # Draw a box around the face
                    cv2.rectangle(im0, (left, top), (right, bottom), (0, 0, 255), 2)

                    # Draw a label with a name below the face
                    cv2.rectangle(im0, (left, bottom - 35), (right, bottom), (0, 0, 255), cv2.FILLED)
